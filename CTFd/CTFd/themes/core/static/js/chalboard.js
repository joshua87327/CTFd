...
function marksolves(cb) {
    $.get(script_root + '/solves', function (data) {
	    ...
	    btn.children("i").remove();
            btn.prepend("<i class='swg swg-bb8-4 swg-6x swg-pull-right'></i>")
        }
        if (cb) {
            cb();
        }
    ...
    $.get(script_root + "/chals", function (data) {
        ...
        for (var i = 0; i <= challenges['game'].length - 1; i++) {
            ...
            if (user_solves.indexOf(chalinfo.id) == -1){
                var chalbutton = $("<button class='btn btn-dark challenge-button w-100 text-truncate pt-3 pb-3 mb-2' value='{0}'><i class='swg swg-clonetrooper-2 swg-6x swg-pull-right'></i></button>".format(chalinfo.id));
            } else {
                var chalbutton = $("<button class='btn btn-dark challenge-button solved-challenge w-100 text-truncate pt-3 pb-3 mb-2' value='{0}'><i class='swg swg-bb8-4 swg-6x swg-pull-right'></i></button>".format(chalinfo.id));
            }
